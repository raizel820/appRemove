...
const getPaginationSettings = (config: PDFConfiguration): PaginationSettings => {
  const settings = {
    singlePageItemsMaxHeight_mm: config?.invoiceSinglePageTableMaxHeight || 200,
    multiPageFirstMaxHeight_mm: config?.invoiceFirstPageTableMaxHeight || 180,
    multiPageNonLastMaxHeight_mm: config?.invoiceOtherPagesTableMaxHeight || 180,
    multiPageLastMaxHeight_mm: config?.invoiceLastPageTableMaxHeight || 150,
    summaryTermsHeight_mm: 50,
    pageMargin_mm: 10,
    a4Height_mm: 297,
  };
  console.log('[getPaginationSettings] Config values:', {
    invoiceSinglePageTableMaxHeight: config?.invoiceSinglePageTableMaxHeight,
    invoiceFirstPageTableMaxHeight: config?.invoiceFirstPageTableMaxHeight,
    invoiceOtherPagesTableMaxHeight: config?.invoiceOtherPagesTableMaxHeight,
    invoiceLastPageTableMaxHeight: config?.invoiceLastPageTableMaxHeight,
  });
  console.log('[getPaginationSettings] Resulting settings:', settings);
  return settings;
};
...
const paginateItems = (items: RowMeasurement[], settings: PaginationSettings): PageDescriptor[] => {
  ...
  console.log('[paginateItems] Settings:', { singlePageItemsMaxHeight_mm, multiPageFirstMaxHeight_mm, multiPageNonLastMaxHeight_mm, multiPageLastMaxHeight_mm });
  console.log('[paginateItems] Items:', {
    count: items.length,
  ...
  if (isSinglePage) {
    console.log('[paginateItems] Using single page mode');
    return [{
      pageNum: 1,
      pageType: 'single',
  ...
  console.log('[paginateItems] Using multi-page mode');
  ...
  console.log('[paginateItems] Reserved last item for last page (isEmpty:', reservedLastItem.isEmptyRow, ')');
  ...
  if (totalHeightWithoutReserved <= multiPageLastMaxHeight_mm) {
    ...
    if (firstPageItems.length > 0) {
      ...
      console.log(`[paginateItems] Created first page ${currentPageNum} with ${firstPageItems.length} items (real: ${firstPageItems.filter(r => !r.isEmptyRow).length}), height: ${firstPageHeight.toFixed(2)}mm`);
      currentPageNum++;
      ...
      console.log(`[paginateItems] Created last page ${currentPageNum} with ${lastPageItems.length} items, height: ${lastPageHeight.toFixed(2)}mm`);
    } else {
    ...
    console.log('[paginateItems] Total pages created:', pages.length);
    return pages;
  }
  ...
  for (let i = 0; i < itemsWithoutReserved.length; i++) {
    ...
    if (currentHeight + item.height > multiPageFirstMaxHeight_mm) {
      ...
      if (itemsForThisPage.length > 0) {
        ...
        console.log(`[paginateItems] Created first page ${currentPageNum} with ${itemsForThisPage.length} items (real: ${itemsForThisPage.filter(r => !r.isEmptyRow).length}), height: ${itemsForThisPageHeight.toFixed(2)}mm`);
        currentPageNum++;
        itemsForThisPage = [];
        currentHeight = 0;
      }
  ...
  if (itemsForThisPage.length > 0) {
    ...
    if (pages.length === 0) {
      ...
      console.log(`[paginateItems] Created first page ${currentPageNum} with 1 item, height: ${firstItem.height.toFixed(2)}mm`);
      currentPageNum++;
      ...
      for (const item of itemsForThisPage) {
        if (currentOtherPageHeight + item.height > multiPageNonLastMaxHeight_mm) {
          ...
          console.log(`[paginateItems] Created other page ${currentPageNum} with ${itemsForOtherPage.length} items (real: ${itemsForOtherPage.filter(r => !r.isEmptyRow).length}), height: ${otherPageHeight.toFixed(2)}mm`);
          currentPageNum++;
          itemsForOtherPage = [];
...
export const calculateTableTotalHeight = (
  ...
  const emptyRows = config?.invoiceEmptyRows ?? 8;

  console.log('[calculateTableTotalHeight] Input:', { actualItems, emptyRows });
  ...
  console.log('[calculateTableTotalHeight] Result:', { totalHeight: totalHeight.toFixed(2), allItems });
  return totalHeight;
};
...
export const generatePreviewHTML = ({
  ...
  invoiceYear: providedInvoiceYear,
  activityProfile
...
<div class="error-container">
  ...
  const tInv = invoiceTranslations[selectedLanguage as keyof typeof invoiceTranslations];
  ...
  const titleFontFamily = fontFamilyMap[config.invoiceTitleFontFamily || "HELVETICA"];
  const clientLabelFontFamily = fontFamilyMap[config.invoiceClientLabelFontFamily || "HELVETICA"];
  const clientValueFontFamily = fontFamilyMap[config.invoiceClientValueFontFamily || "HELVETICA"];
  const dateFontFamily = fontFamilyMap[config.invoiceDateFontFamily || "HELVETICA"];
  const tableLabelFontFamily = fontFamilyMap[config.invoiceTableLabelFontFamily || "HELVETICA"];
  const tableValueFontFamily = fontFamilyMap[config.invoiceTableValueFontFamily || "HELVETICA"];
  const termsFontFamily = fontFamilyMap[config.invoiceTermsFontFamily || "HELVETICA"];
  const pageNumberFontFamily = fontFamilyMap[config.invoicePageNumberFontFamily || "HELVETICA"];

  const lastCustomer = customers[0];
  const todayDate = getTodayDate();
  const currentYear = providedInvoiceYear || getCurrentYear();
  const invoiceNumber = providedInvoiceNumber || getInvoiceNumber();
  ...
  <div class="invoice-top-body">
    <div class="invoice-title" dir="${selectedLanguage === 'AR' ? 'rtl' : 'ltr'}">${tInv.invoiceNumber} : ${invoiceNumber} / ${currentYear}</div>
    ...
    <div class="date-section">
      ${config.invoiceLocationDateLabelText || tInv.datePrefix} ${todayDate}
    </div>
  </div>
  `;
  ...
  <div class="items-table-wrapper" style="${maxHeightStyle}">
    ...
    <thead>
      <tr>
        <th style="width: ${config.invoiceColNumberWidth ?? 13.23}mm; padding: ${(config.invoiceColNumberPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.number}</th>
        <th style="width: ${config.invoiceColFamilyWidth ?? 39.69}mm; padding: ${(config.invoiceColFamilyPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.family}</th>
        <th style="width: ${config.invoiceColModelWidth ?? 39.69}mm; padding: ${(config.invoiceColModelPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.model}</th>
        <th style="width: ${config.invoiceColBrandWidth ?? 31.75}mm; padding: ${(config.invoiceColBrandPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.brand}</th>
        <th style="width: ${config.invoiceColQuantityWidth ?? 21.17}mm; padding: ${(config.invoiceColQuantityPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.quantity}</th>
        <th style="width: ${config.invoiceColPriceUnitWidth ?? 31.75}mm; padding: ${(config.invoiceColPriceUnitPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.priceUnit}</th>
        <th style="width: ${config.invoiceColPriceTotalWidth ?? 31.75}mm; padding: ${(config.invoiceColPriceTotalPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.priceTotal}</th>
      </tr>
    </thead>
    <tbody>
      ${pageItems.map((item: any) => {
        if (item.type === 'model') {
          ...
          return `
      <tr>
        <td style="padding: ${(config.invoiceColNumberPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${index + 1}</td>
        <td style="padding: ${(config.invoiceColFamilyPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${model.family?.name || ""}</td>
        <td style="padding: ${(config.invoiceColModelPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${model.name}</td>
        <td style="padding: ${(config.invoiceColBrandPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${brand}</td>
        <td style="padding: ${(config.invoiceColQuantityPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${quantity}</td>
        <td style="padding: ${(config.invoiceColPriceUnitPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${formatPrice(pricePerUnit, selectedCurrency)} ${selectedCurrency}</td>
        <td style="padding: ${(config.invoiceColPriceTotalPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${formatPrice(priceExclTax, selectedCurrency)} ${selectedCurrency}</td>
      </tr>`;
        } else {
          return `
      <tr>
        <td style="border: 1px solid #000; padding: ${(config.invoiceColNumberPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.invoiceColFamilyPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.invoiceColModelPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.invoiceColBrandPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.invoiceColQuantityPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.invoiceColPriceUnitPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.invoiceColPriceTotalPadding ?? 5.61) / 2}mm;">&nbsp;</td>
      </tr>`;
        }
      }).join('')}
    </tbody>
  ...
  const generateFooterHTML = () => {
    const signaturePosition = config.invoiceSignaturePosition || "right";
    const signatureCount = config.invoiceSignatureCount || 2;
    const signatureLineWidth = config.invoiceSignatureLineWidth || 150;
    let signatureHTML = '';

    if (signatureCount === 1) {
      if (config.includeLeftSignature !== false) {
      ...
      <div class="signature-line" style="${justifyContentStyle}" dir="ltr">
        <div class="signature-box" style="width: auto; text-align: center;">
          ...
          <div class="signature-label" style="text-align: center;">${config.invoiceSignatureLeftLabelText || tInv.signature.manager}</div>
        </div>
      </div>`;
      }
    } else {
      if (config.includeLeftSignature !== false || config.includeRightSignature !== false) {
        signatureHTML = '<div class="signature-line" dir="ltr">';
        if (config.includeLeftSignature !== false) {
          signatureHTML += `
        <div class="signature-box" style="position: relative; left: ${config.leftSignatureOffsetX || 0}px; top: ${config.leftSignatureOffsetY || 0}px;">
          <div class="signature-line-visual" style="width: ${signatureLineWidth}px;"></div>
          <div class="signature-label" style="text-align: center;">${config.invoiceSignatureLeftLabelText || tInv.signature.manager}</div>
        </div>`;
        }
        if (config.includeRightSignature !== false) {
          signatureHTML += `
        <div class="signature-box" style="position: relative; left: ${config.rightSignatureOffsetX || 0}px; top: ${config.rightSignatureOffsetY || 0}px;">
          <div class="signature-line-visual" style="width: ${signatureLineWidth}px;"></div>
          <div class="signature-label" style="text-align: center;">${config.invoiceSignatureRightLabelText || tInv.signature.client}</div>
        </div>`;
        }
        signatureHTML += '</div>';
      }
    ...
    if (config.invoiceTermsIncludeDelivery !== false) {
      const timeText = formatTimePeriod(config.invoiceTermsDeliveryTimeFrom || 3, config.invoiceTermsDeliveryTimeTo || null, config.invoiceTermsDeliveryTimeUnit || 'months', langCode);
    ...
    if (config.invoiceTermsIncludeValidity !== false) {
      const timeText = formatTimePeriod(config.invoiceTermsValidityTime || 3, config.invoiceTermsValidityTimeTo || null, config.invoiceTermsValidityTimeUnit || 'months', langCode);
      termsList.push(`${tInv.terms.validityTitle}: ${timeText}`);
    }
    if (config.invoiceTermsIncludeWarranty !== false) {
      const timeText = formatTimePeriod(config.invoiceTermsWarrantyTime || 12, config.invoiceTermsWarrantyTimeTo || null, config.invoiceTermsWarrantyTimeUnit || 'months', langCode);
      termsList.push(`${tInv.terms.warrantyTitle}: ${timeText}`);
    }
    if (config.invoiceTermsIncludeConfirmation !== false) {
      termsList.push(config.invoiceTermsDeliveryCondition === 'after_payment' ? tInv.terms.afterPayment : tInv.terms.afterOrderConfirmation);
    }
    ...
    if (config.invoiceTermsIncludePriceInWords !== false) {
  ...
  <div class="footer-section" dir="${selectedLanguage === 'AR' ? 'rtl' : 'ltr'}">
    <div class="terms-section">
      <div class="terms-title">${config.invoiceTermsTitle || tInv.terms.title}</div>
  ...
  let pagesHTML = '';
  console.log('[generatePreviewHTML] Total pages to generate:', pages.length);
  for (let i = 0; i < pages.length; i++) {
    const page = pages[i];
    const isLastPage = page.pageType === 'multi-last' || page.pageType === 'single';
    console.log(`[generatePreviewHTML] Page ${page.pageNum}:`, {
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .invoice-top-body { margin-bottom: ${config.factureNumberMargin ? config.factureNumberMargin / 3.78 : 0}mm; direction: ${config.invoiceTopBodyDirection === 'RTL' ? 'rtl' : 'ltr'}; }
    .invoice-title { font-family: ${titleFontFamily}; font-size: ${config.invoiceTitleFontSize || 16}px; color: ${config.invoiceTitleTextColor || "#000000"}; font-weight: ${config.invoiceTitleBold ? 'bold' : 'normal'}; text-decoration: ${config.invoiceTitleUnderline ? 'underline' : 'none'}; margin-bottom: ${10 * (config.invoiceTitleLineSpacing || 0.5) / 3.78}mm; line-height: ${config.invoiceTitleLineSpacing || 0.5}; text-align: ${config.invoiceTitlePosition || 'center'}; letter-spacing: ${config.invoiceTitleLetterSpacing || 0}px; }
    .customer-section { display: flex; gap: 8mm; padding: 3mm; border-bottom: 1px solid #e0e0e0; text-align: ${config.invoiceClientInfoPosition || 'left'}; background: #f9f9f9; }
    ...
    .customer-label { font-family: ${clientLabelFontFamily}; font-size: ${config.invoiceClientLabelFontSize || 12}px; color: ${config.invoiceClientLabelTextColor || "#000000"}; font-weight: ${config.invoiceClientLabelBold ? 'bold' : 'normal'}; text-decoration: ${config.invoiceClientLabelUnderline ? 'underline' : 'none'}; line-height: ${config.invoiceClientLabelLineSpacing || 1}; letter-spacing: ${config.invoiceClientLabelLetterSpacing || 0}px; }
    .customer-value { font-family: ${clientValueFontFamily}; font-size: ${config.invoiceClientValueFontSize || 12}px; color: ${config.invoiceClientValueTextColor || "#000000"}; font-weight: ${config.invoiceClientValueBold ? 'bold' : 'normal'}; text-decoration: ${config.invoiceClientValueUnderline ? 'underline' : 'none'}; line-height: ${config.invoiceClientValueLineSpacing || 1}; letter-spacing: ${config.invoiceClientValueLetterSpacing || 0}px; }
    .customer-field { display: flex; gap: 4mm; }
    .date-section { margin-bottom: ${config.dateMargin ? config.dateMargin / 3.78 : 1.32}mm; font-family: ${dateFontFamily}; font-size: ${config.invoiceDateFontSize || 12}px; color: ${config.invoiceDateTextColor || "#000000"}; font-weight: ${config.invoiceDateBold ? 'bold' : 'normal'}; text-decoration: ${config.invoiceDateUnderline ? 'underline' : 'none'}; line-height: ${config.invoiceDateLineSpacing || 1}; letter-spacing: ${config.invoiceDateLetterSpacing || 0}px; text-align: ${config.invoiceDatePosition || 'left'}; }
    ...
    .items-table th, .items-table td { border: 1px solid #000; text-align: ${config.invoiceTableTextAlignment || 'left'}; direction: ${config.invoiceTableDirection === 'RTL' ? 'rtl' : 'ltr'}; }
    .items-table th { font-family: ${tableLabelFontFamily}; font-size: ${config.invoiceTableLabelFontSize || 11}px; color: ${config.invoiceTableLabelTextColor || "#000000"}; font-weight: ${config.invoiceTableLabelBold ? 'bold' : 'normal'}; text-decoration: ${config.invoiceTableLabelUnderline ? 'underline' : 'none'}; line-height: ${config.invoiceTableLabelLineSpacing || 1}; letter-spacing: ${config.invoiceTableLabelLetterSpacing || 0}px; background: #f0f0f0; }
    .items-table td { font-family: ${tableValueFontFamily}; font-size: ${config.invoiceTableValueFontSize || 11}px; color: ${config.invoiceTableValueTextColor || "#000000"}; font-weight: ${config.invoiceTableValueBold ? 'bold' : 'normal'}; text-decoration: ${config.invoiceTableValueUnderline ? 'underline' : 'none'}; line-height: ${config.invoiceTableValueLineSpacing || 1}; letter-spacing: ${config.invoiceTableValueLetterSpacing || 0}px; vertical-align: top; }
    ...
    .totals-table td { padding: 2.12mm; border: 1px solid #000; font-family: ${tableValueFontFamily}; font-size: ${config.invoiceTableValueFontSize || 11}px; color: ${config.invoiceTableValueTextColor || "#000000"}; font-weight: ${config.invoiceTableValueBold ? 'bold' : 'normal'}; text-decoration: ${config.invoiceTableValueUnderline ? 'underline' : 'none'}; line-height: ${config.invoiceTableValueLineSpacing || 1}; letter-spacing: ${config.invoiceTableValueLetterSpacing || 0}px; }
    ...
    .terms-section { margin-bottom: ${config.termsSectionMargin ? config.termsSectionMargin / 3.78 : 1.32}mm; font-family: ${termsFontFamily}; font-size: ${config.invoiceTermsFontSize || 11}px; color: ${config.invoiceTermsTextColor || "#000000"}; font-weight: ${config.invoiceTermsBold ? 'bold' : 'normal'}; text-decoration: ${config.invoiceTermsUnderline ? 'underline' : 'none'}; line-height: ${config.invoiceTermsLineSpacing || 1}; letter-spacing: ${config.invoiceTermsLetterSpacing || 0}px; text-align: ${config.invoiceTermsPosition || 'left'}; }
    ...
    .page-number { position: absolute; bottom: ${5 + (config.invoicePageNumberOffsetY || 0)}mm; ${config.invoicePageNumberPosition === 'left' ? 'left: 10mm;' : ''} ${config.invoicePageNumberPosition === 'right' ? 'right: 10mm;' : ''} ${config.invoicePageNumberPosition === 'center' || !config.invoicePageNumberPosition ? 'left: 50%; transform: translateX(-50%);' : ''} ${config.invoicePageNumberPosition !== 'center' && config.invoicePageNumberPosition ? `transform: translateX(${config.invoicePageNumberOffsetX || 0}px);` : ''} font-family: ${pageNumberFontFamily}; font-size: ${config.invoicePageNumberFontSize || 10}px; color: ${config.invoicePageNumberTextColor || "#666"}; text-align: center; }
...
export const generatePDFFromHTML = async (
  previewHTML: string,
  fileName: string = `Invoice_${getTodayDate().replace(/\//g, '-')}.pdf`
): Promise<void> => {
