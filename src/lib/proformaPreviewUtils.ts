...
export const getProformaNumber = (): string => {
  return "1";
};
...
const getPaginationSettings = (config: PDFConfiguration): PaginationSettings => {
  const settings = {
    singlePageItemsMaxHeight_mm: config?.proformaSinglePageTableMaxHeight || 200,
    multiPageFirstMaxHeight_mm: config?.proformaFirstPageTableMaxHeight || 180,
    multiPageNonLastMaxHeight_mm: config?.proformaOtherPagesTableMaxHeight || 180,
    multiPageLastMaxHeight_mm: config?.proformaLastPageTableMaxHeight || 150,
    summaryTermsHeight_mm: 50,
    pageMargin_mm: 10,
    a4Height_mm: 297,
  };
  console.log('[getPaginationSettings - Proforma] Config values:', {
    proformaSinglePageTableMaxHeight: config?.proformaSinglePageTableMaxHeight,
    proformaFirstPageTableMaxHeight: config?.proformaFirstPageTableMaxHeight,
    proformaOtherPagesTableMaxHeight: config?.proformaOtherPagesTableMaxHeight,
    proformaLastPageTableMaxHeight: config?.proformaLastPageTableMaxHeight,
  });
  console.log('[getPaginationSettings - Proforma] Resulting settings:', settings);
  return settings;
};
...
const paginateItems = (items: RowMeasurement[], settings: PaginationSettings): PageDescriptor[] => {
  ...
  console.log('[paginateItems - Proforma] Settings:', { singlePageItemsMaxHeight_mm, multiPageFirstMaxHeight_mm, multiPageNonLastMaxHeight_mm, multiPageLastMaxHeight_mm });
  console.log('[paginateItems - Proforma] Items:', {
    count: items.length,
  ...
  if (isSinglePage) {
    console.log('[paginateItems - Proforma] Using single page mode');
    return [{
      pageNum: 1,
      pageType: 'single',
  ...
  console.log('[paginateItems - Proforma] Using multi-page mode');
  ...
  console.log('[paginateItems - Proforma] Reserved last item for last page (isEmpty:', reservedLastItem.isEmptyRow, ')');
  ...
  if (totalHeightWithoutReserved <= multiPageLastMaxHeight_mm) {
    ...
    if (firstPageItems.length > 0) {
      ...
      console.log(`[paginateItems - Proforma] Created first page ${currentPageNum} with ${firstPageItems.length} items (real: ${firstPageItems.filter(r => !r.isEmptyRow).length}), height: ${firstPageHeight.toFixed(2)}mm`);
      currentPageNum++;
      ...
      console.log(`[paginateItems - Proforma] Created last page ${currentPageNum} with ${lastPageItems.length} items, height: ${lastPageHeight.toFixed(2)}mm`);
    } else {
    ...
    console.log('[paginateItems - Proforma] Total pages created:', pages.length);
    return pages;
  }
  ...
  for (let i = 0; i < itemsWithoutReserved.length; i++) {
    ...
    if (currentHeight + item.height > multiPageFirstMaxHeight_mm) {
      ...
      if (itemsForThisPage.length > 0) {
        ...
        console.log(`[paginateItems - Proforma] Created first page ${currentPageNum} with ${itemsForThisPage.length} items (real: ${itemsForThisPage.filter(r => !r.isEmptyRow).length}), height: ${itemsForThisPageHeight.toFixed(2)}mm`);
        currentPageNum++;
        itemsForThisPage = [];
        currentHeight = 0;
      }
  ...
  if (itemsForThisPage.length > 0) {
    ...
    if (pages.length === 0) {
      ...
      console.log(`[paginateItems - Proforma] Created first page ${currentPageNum} with 1 item, height: ${firstItem.height.toFixed(2)}mm`);
      currentPageNum++;
      ...
      for (const item of itemsForThisPage) {
        if (currentOtherPageHeight + item.height > multiPageNonLastMaxHeight_mm) {
          ...
          console.log(`[paginateItems - Proforma] Created other page ${currentPageNum} with ${itemsForOtherPage.length} items (real: ${itemsForOtherPage.filter(r => !r.isEmptyRow).length}), height: ${otherPageHeight.toFixed(2)}mm`);
          currentPageNum++;
          itemsForOtherPage = [];
...
export const calculateTableTotalHeight = (
  ...
  const emptyRows = config?.proformaEmptyRows ?? 8;

  console.log('[calculateTableTotalHeight - Proforma] Input:', { actualItems, emptyRows });
  ...
  console.log('[calculateTableTotalHeight - Proforma] Result:', { totalHeight: totalHeight.toFixed(2), allItems });
  return totalHeight;
};
...
export const getQrCodeDataUrls = async (
  ...
  try {
    ...
    // Format proforma number: PRO-005/2026
    const formattedProformaNumber = proformaNumber 
      ? `PRO-${String(proformaNumber).padStart(3, '0')}/${proformaYear || new Date().getFullYear()}`
      : `PRO-001/${new Date().getFullYear()}`;
    
    const fileInfo: FileInfo = {
      type: "Proforma",
      number: formattedProformaNumber,
...
export const generatePreviewHTML = ({
  ...
  labelsFr,
  labelsAr,
  proformaNumber: providedProformaNumber,
  proformaYear: providedProformaYear,
  activityProfile
...
<div class="error-container">
  ...
  const tInv = proformaTranslations[selectedLanguage as keyof typeof proformaTranslations];
  ...
  const titleFontFamily = fontFamilyMap[config.proformaTitleFontFamily || "HELVETICA"];
  const clientLabelFontFamily = fontFamilyMap[config.proformaClientLabelFontFamily || "HELVETICA"];
  const clientValueFontFamily = fontFamilyMap[config.proformaClientValueFontFamily || "HELVETICA"];
  const dateFontFamily = fontFamilyMap[config.proformaDateFontFamily || "HELVETICA"];
  const tableLabelFontFamily = fontFamilyMap[config.proformaTableLabelFontFamily || "HELVETICA"];
  const tableValueFontFamily = fontFamilyMap[config.proformaTableValueFontFamily || "HELVETICA"];
  const termsFontFamily = fontFamilyMap[config.proformaTermsFontFamily || "HELVETICA"];
  const pageNumberFontFamily = fontFamilyMap[config.proformaPageNumberFontFamily || "HELVETICA"];

  const lastCustomer = customers[0];
  const todayDate = getTodayDate();
  const currentYear = providedProformaYear || getCurrentYear();
  const proformaNumber = providedProformaNumber || getProformaNumber();
  ...
  <div class="proforma-top-body">
    <div class="proforma-title" dir="${selectedLanguage === 'AR' ? 'rtl' : 'ltr'}">${tInv.proformaNumber} : ${proformaNumber} / ${currentYear}</div>
    ...
    <div class="date-section">
      ${config.proformaLocationDateLabelText || tInv.datePrefix} ${todayDate}
    </div>
  </div>
  `;
  ...
  <div class="items-table-wrapper" style="${maxHeightStyle}">
    ...
    <thead>
      <tr>
        <th style="width: ${config.proformaColNumberWidth ?? 13.23}mm; padding: ${(config.proformaColNumberPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.number}</th>
        <th style="width: ${config.proformaColFamilyWidth ?? 39.69}mm; padding: ${(config.proformaColFamilyPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.family}</th>
        <th style="width: ${config.proformaColModelWidth ?? 39.69}mm; padding: ${(config.proformaColModelPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.model}</th>
        <th style="width: ${config.proformaColBrandWidth ?? 31.75}mm; padding: ${(config.proformaColBrandPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.brand}</th>
        <th style="width: ${config.proformaColQuantityWidth ?? 21.17}mm; padding: ${(config.proformaColQuantityPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.quantity}</th>
        <th style="width: ${config.proformaColPriceUnitWidth ?? 31.75}mm; padding: ${(config.proformaColPriceUnitPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.priceUnit}</th>
        <th style="width: ${config.proformaColPriceTotalWidth ?? 31.75}mm; padding: ${(config.proformaColPriceTotalPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.priceTotal}</th>
      </tr>
    </thead>
    <tbody>
      ${pageItems.map((item: any) => {
        if (item.type === 'model') {
          ...
          return `
      <tr>
        <td style="padding: ${(config.proformaColNumberPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${index + 1}</td>
        <td style="padding: ${(config.proformaColFamilyPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${model.family?.name || ""}</td>
        <td style="padding: ${(config.proformaColModelPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${model.name}</td>
        <td style="padding: ${(config.proformaColBrandPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${brand}</td>
        <td style="padding: ${(config.proformaColQuantityPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${quantity}</td>
        <td style="padding: ${(config.proformaColPriceUnitPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${formatPrice(pricePerUnit, selectedCurrency)} ${selectedCurrency}</td>
        <td style="padding: ${(config.proformaColPriceTotalPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${formatPrice(priceExclTax, selectedCurrency)} ${selectedCurrency}</td>
      </tr>`;
        } else {
          return `
      <tr>
        <td style="border: 1px solid #000; padding: ${(config.proformaColNumberPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.proformaColFamilyPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.proformaColModelPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.proformaColBrandPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.proformaColQuantityPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.proformaColPriceUnitPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.proformaColPriceTotalPadding ?? 5.61) / 2}mm;">&nbsp;</td>
      </tr>`;
        }
      }).join('')}
    </tbody>
  ...
  const generateFooterHTML = () => {
    const signaturePosition = config.proformaSignaturePosition || "right";
    const signatureCount = config.proformaSignatureCount || 2;
    const signatureLineWidth = config.proformaSignatureLineWidth || 150;
    let signatureHTML = '';

    if (signatureCount === 1) {
      if (config.proformaIncludeLeftSignature !== false) {
      ...
      <div class="signature-line" style="${justifyContentStyle}" dir="ltr">
        <div class="signature-box" style="width: auto; text-align: center;">
          ...
          <div class="signature-label" style="text-align: center;">${config.proformaSignatureLeftLabelText || tInv.signature.manager}</div>
        </div>
      </div>`;
      }
    } else {
      if (config.proformaIncludeLeftSignature !== false || config.proformaIncludeRightSignature !== false) {
        signatureHTML = '<div class="signature-line" dir="ltr">';
        if (config.proformaIncludeLeftSignature !== false) {
          signatureHTML += `
        <div class="signature-box" style="position: relative; left: ${config.proformaLeftSignatureOffsetX || 0}px; top: ${config.proformaLeftSignatureOffsetY || 0}px;">
          <div class="signature-line-visual" style="width: ${signatureLineWidth}px;"></div>
          <div class="signature-label" style="text-align: center;">${config.proformaSignatureLeftLabelText || tInv.signature.manager}</div>
        </div>`;
        }
        if (config.proformaIncludeRightSignature !== false) {
          signatureHTML += `
        <div class="signature-box" style="position: relative; left: ${config.proformaRightSignatureOffsetX || 0}px; top: ${config.proformaRightSignatureOffsetY || 0}px;">
          <div class="signature-line-visual" style="width: ${signatureLineWidth}px;"></div>
          <div class="signature-label" style="text-align: center;">${config.proformaSignatureRightLabelText || tInv.signature.client}</div>
        </div>`;
        }
        signatureHTML += '</div>';
      }
    ...
    if (config.proformaTermsIncludeDelivery !== false) {
      const timeText = formatTimePeriod(config.proformaTermsDeliveryTimeFrom || 3, config.proformaTermsDeliveryTimeTo || null, config.proformaTermsDeliveryTimeUnit || 'months', langCode);
      termsList.push(`${tInv.terms.deliveryAfterOrder}: ${timeText} ${tInv.terms.afterOrderConfirmation.toLowerCase()}`);
    }
    if (config.proformaTermsIncludeValidity !== false) {
      const timeText = formatTimePeriod(config.proformaTermsValidityTime || 3, config.proformaTermsValidityTimeTo || null, config.proformaTermsValidityTimeUnit || 'months', langCode);
      termsList.push(`${tInv.terms.validityTitle}: ${timeText}`);
    }
    if (config.proformaTermsIncludeWarranty !== false) {
      const timeText = formatTimePeriod(config.proformaTermsWarrantyTime || 12, config.proformaTermsWarrantyTimeTo || null, config.proformaTermsWarrantyTimeUnit || 'months', langCode);
      termsList.push(`${tInv.terms.warrantyTitle}: ${timeText}`);
    }
    if (config.proformaTermsIncludeConfirmation !== false) {
      termsList.push(config.proformaTermsDeliveryCondition === 'after_payment' ? tInv.terms.afterPayment : tInv.terms.afterOrderConfirmation);
    }
    if (config.proformaTermsIncludePayment !== false) {
      const paymentMethod = config.proformaTermsPaymentMethod || 'split_payment';
      ...
      if (paymentMethod === 'split_payment') {
        const percent = config.proformaTermsSplitPaymentPercent || 50;
    ...
    if (config.proformaTermsIncludePriceInWords !== false) {
  ...
  <div class="footer-section" dir="${selectedLanguage === 'AR' ? 'rtl' : 'ltr'}">
    <div class="terms-section">
      <div class="terms-title">${config.proformaTermsTitle || tInv.terms.title}</div>
  ...
  let pagesHTML = '';
  console.log('[generatePreviewHTML - Proforma] Total pages to generate:', pages.length);
  for (let i = 0; i < pages.length; i++) {
    const page = pages[i];
    const isLastPage = page.pageType === 'multi-last' || page.pageType === 'single';
    console.log(`[generatePreviewHTML - Proforma] Page ${page.pageNum}:`, {
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .proforma-top-body { margin-bottom: ${config.factureNumberMargin ? config.factureNumberMargin / 3.78 : 0}mm; direction: ${config.proformaTopBodyDirection === 'RTL' ? 'rtl' : 'ltr'}; }
    .proforma-title { font-family: ${titleFontFamily}; font-size: ${config.proformaTitleFontSize || 16}px; color: ${config.proformaTitleTextColor || "#000000"}; font-weight: ${config.proformaTitleBold ? 'bold' : 'normal'}; text-decoration: ${config.proformaTitleUnderline ? 'underline' : 'none'}; margin-bottom: ${10 * (config.proformaTitleLineSpacing || 0.5) / 3.78}mm; line-height: ${config.proformaTitleLineSpacing || 0.5}; text-align: ${config.proformaTitlePosition || 'center'}; letter-spacing: ${config.proformaTitleLetterSpacing || 0}px; }
    .customer-section { display: flex; gap: 8mm; padding: 3mm; border-bottom: 1px solid #e0e0e0; text-align: ${config.proformaClientInfoPosition || 'left'}; background: #f9f9f9; }
    ...
    .customer-label { font-family: ${clientLabelFontFamily}; font-size: ${config.proformaClientLabelFontSize || 12}px; color: ${config.proformaClientLabelTextColor || "#000000"}; font-weight: ${config.proformaClientLabelBold ? 'bold' : 'normal'}; text-decoration: ${config.proformaClientLabelUnderline ? 'underline' : 'none'}; line-height: ${config.proformaClientLabelLineSpacing || 1}; letter-spacing: ${config.proformaClientLabelLetterSpacing || 0}px; }
    .customer-value { font-family: ${clientValueFontFamily}; font-size: ${config.proformaClientValueFontSize || 12}px; color: ${config.proformaClientValueTextColor || "#000000"}; font-weight: ${config.proformaClientValueBold ? 'bold' : 'normal'}; text-decoration: ${config.proformaClientValueUnderline ? 'underline' : 'none'}; line-height: ${config.proformaClientValueLineSpacing || 1}; letter-spacing: ${config.proformaClientValueLetterSpacing || 0}px; }
    .customer-field { display: flex; gap: 4mm; }
    .date-section { margin-bottom: ${config.dateMargin ? config.dateMargin / 3.78 : 1.32}mm; font-family: ${dateFontFamily}; font-size: ${config.proformaDateFontSize || 12}px; color: ${config.proformaDateTextColor || "#000000"}; font-weight: ${config.proformaDateBold ? 'bold' : 'normal'}; text-decoration: ${config.proformaDateUnderline ? 'underline' : 'none'}; line-height: ${config.proformaDateLineSpacing || 1}; letter-spacing: ${config.proformaDateLetterSpacing || 0}px; text-align: ${config.proformaDatePosition || 'left'}; }
    ...
    .items-table th, .items-table td { border: 1px solid #000; text-align: ${config.proformaTableTextAlignment || 'left'}; direction: ${config.proformaTableDirection === 'RTL' ? 'rtl' : 'ltr'}; }
    .items-table th { font-family: ${tableLabelFontFamily}; font-size: ${config.proformaTableLabelFontSize || 11}px; color: ${config.proformaTableLabelTextColor || "#000000"}; font-weight: ${config.proformaTableLabelBold ? 'bold' : 'normal'}; text-decoration: ${config.proformaTableLabelUnderline ? 'underline' : 'none'}; line-height: ${config.proformaTableLabelLineSpacing || 1}; letter-spacing: ${config.proformaTableLabelLetterSpacing || 0}px; background: #f0f0f0; }
    .items-table td { font-family: ${tableValueFontFamily}; font-size: ${config.proformaTableValueFontSize || 11}px; color: ${config.proformaTableValueTextColor || "#000000"}; font-weight: ${config.proformaTableValueBold ? 'bold' : 'normal'}; text-decoration: ${config.proformaTableValueUnderline ? 'underline' : 'none'}; line-height: ${config.proformaTableValueLineSpacing || 1}; letter-spacing: ${config.proformaTableValueLetterSpacing || 0}px; vertical-align: top; }
    ...
    .totals-table td { padding: 2.12mm; border: 1px solid #000; font-family: ${tableValueFontFamily}; font-size: ${config.proformaTableValueFontSize || 11}px; color: ${config.proformaTableValueTextColor || "#000000"}; font-weight: ${config.proformaTableValueBold ? 'bold' : 'normal'}; text-decoration: ${config.proformaTableValueUnderline ? 'underline' : 'none'}; line-height: ${config.proformaTableValueLineSpacing || 1}; letter-spacing: ${config.proformaTableValueLetterSpacing || 0}px; }
    ...
    .terms-section { margin-bottom: ${config.termsSectionMargin ? config.termsSectionMargin / 3.78 : 1.32}mm; font-family: ${termsFontFamily}; font-size: ${config.proformaTermsFontSize || 11}px; color: ${config.proformaTermsTextColor || "#000000"}; font-weight: ${config.proformaTermsBold ? 'bold' : 'normal'}; text-decoration: ${config.proformaTermsUnderline ? 'underline' : 'none'}; line-height: ${config.proformaTermsLineSpacing || 1}; letter-spacing: ${config.proformaTermsLetterSpacing || 0}px; text-align: ${config.proformaTermsPosition || 'left'}; }
    ...
    .page-number { position: absolute; bottom: ${5 + (config.proformaPageNumberOffsetY || 0)}mm; ${config.proformaPageNumberPosition === 'left' ? 'left: 10mm;' : ''} ${config.proformaPageNumberPosition === 'right' ? 'right: 10mm;' : ''} ${config.proformaPageNumberPosition === 'center' || !config.proformaPageNumberPosition ? 'left: 50%; transform: translateX(-50%);' : ''} ${config.proformaPageNumberPosition !== 'center' && config.proformaPageNumberPosition ? `transform: translateX(${config.proformaPageNumberOffsetX || 0}px);` : ''} font-family: ${pageNumberFontFamily}; font-size: ${config.proformaPageNumberFontSize || 10}px; color: ${config.proformaPageNumberTextColor || "#666"}; text-align: center; }
...
export const generatePDFFromHTML = async (
  previewHTML: string,
  fileName: string = `Proforma_${getTodayDate().replace(/\//g, '-')}.pdf`
): Promise<void> => {
