...
export const getDeliveryNoteNumber = (): string => {
  return "1";
};
...
const getPaginationSettings = (config: PDFConfiguration): PaginationSettings => {
  const settings = {
    singlePageItemsMaxHeight_mm: config?.deliveryNoteSinglePageTableMaxHeight || 200,
    multiPageFirstMaxHeight_mm: config?.deliveryNoteFirstPageTableMaxHeight || 180,
    multiPageNonLastMaxHeight_mm: config?.deliveryNoteOtherPagesTableMaxHeight || 180,
    multiPageLastMaxHeight_mm: config?.deliveryNoteLastPageTableMaxHeight || 150,
    summaryTermsHeight_mm: 50,
    pageMargin_mm: 10,
    a4Height_mm: 297,
  };
  console.log('[getPaginationSettings - DeliveryNote] Config values:', {
    deliveryNoteSinglePageTableMaxHeight: config?.deliveryNoteSinglePageTableMaxHeight,
    deliveryNoteFirstPageTableMaxHeight: config?.deliveryNoteFirstPageTableMaxHeight,
    deliveryNoteOtherPagesTableMaxHeight: config?.deliveryNoteOtherPagesTableMaxHeight,
    deliveryNoteLastPageTableMaxHeight: config?.deliveryNoteLastPageTableMaxHeight,
  });
  console.log('[getPaginationSettings - DeliveryNote] Resulting settings:', settings);
  return settings;
};
...
const paginateItems = (items: RowMeasurement[], settings: PaginationSettings): PageDescriptor[] => {
  ...
  console.log('[paginateItems - DeliveryNote] Settings:', { singlePageItemsMaxHeight_mm, multiPageFirstMaxHeight_mm, multiPageNonLastMaxHeight_mm, multiPageLastMaxHeight_mm });
  console.log('[paginateItems - DeliveryNote] Items:', {
    count: items.length,
  ...
  if (isSinglePage) {
    console.log('[paginateItems - DeliveryNote] Using single page mode');
    return [{
      pageNum: 1,
      pageType: 'single',
  ...
  console.log('[paginateItems - DeliveryNote] Using multi-page mode');
  ...
  console.log('[paginateItems - DeliveryNote] Reserved last item for last page (isEmpty:', reservedLastItem.isEmptyRow, ')');
  ...
  if (totalHeightWithoutReserved <= multiPageLastMaxHeight_mm) {
    ...
    if (firstPageItems.length > 0) {
      ...
      console.log(`[paginateItems - DeliveryNote] Created first page ${currentPageNum} with ${firstPageItems.length} items (real: ${firstPageItems.filter(r => !r.isEmptyRow).length}), height: ${firstPageHeight.toFixed(2)}mm`);
      currentPageNum++;
      ...
      console.log(`[paginateItems - DeliveryNote] Created last page ${currentPageNum} with ${lastPageItems.length} items, height: ${lastPageHeight.toFixed(2)}mm`);
    } else {
    ...
    console.log('[paginateItems - DeliveryNote] Total pages created:', pages.length);
    return pages;
  }
  ...
  for (let i = 0; i < itemsWithoutReserved.length; i++) {
    ...
    if (currentHeight + item.height > multiPageFirstMaxHeight_mm) {
      ...
      if (itemsForThisPage.length > 0) {
        ...
        console.log(`[paginateItems - DeliveryNote] Created first page ${currentPageNum} with ${itemsForThisPage.length} items (real: ${itemsForThisPage.filter(r => !r.isEmptyRow).length}), height: ${itemsForThisPageHeight.toFixed(2)}mm`);
        currentPageNum++;
        itemsForThisPage = [];
        currentHeight = 0;
      }
  ...
  if (itemsForThisPage.length > 0) {
    ...
    if (pages.length === 0) {
      ...
      console.log(`[paginateItems - DeliveryNote] Created first page ${currentPageNum} with 1 item, height: ${firstItem.height.toFixed(2)}mm`);
      currentPageNum++;
      ...
      for (const item of itemsForThisPage) {
        if (currentOtherPageHeight + item.height > multiPageNonLastMaxHeight_mm) {
          ...
          console.log(`[paginateItems - DeliveryNote] Created other page ${currentPageNum} with ${itemsForOtherPage.length} items (real: ${itemsForOtherPage.filter(r => !r.isEmptyRow).length}), height: ${otherPageHeight.toFixed(2)}mm`);
          currentPageNum++;
          itemsForOtherPage = [];
...
export const calculateTableTotalHeight = (
  ...
  const emptyRows = config?.deliveryNoteEmptyRows ?? 8;

  console.log('[calculateTableTotalHeight - DeliveryNote] Input:', { actualItems, emptyRows });
  ...
  console.log('[calculateTableTotalHeight - DeliveryNote] Result:', { totalHeight: totalHeight.toFixed(2), allItems });
  return totalHeight;
};
...
export const getQrCodeDataUrls = async (
  ...
  try {
    ...
    // Format delivery note number: DN-005/2026
    const formattedDeliveryNoteNumber = deliveryNoteNumber 
      ? `DN-${String(deliveryNoteNumber).padStart(3, '0')}/${deliveryNoteYear || new Date().getFullYear()}`
      : `DN-001/${new Date().getFullYear()}`;
    
    const fileInfo: FileInfo = {
      type: "DeliveryNote",
      number: formattedDeliveryNoteNumber,
    ...
    const qrCodeData: QRCodeData = {
      ...
      file: fileInfo,
      items: itemsData,
      terms: "Test delivery note terms",
...
export const generatePreviewHTML = ({
  ...
  labelsFr,
  labelsAr,
  deliveryNoteNumber: providedDeliveryNoteNumber,
  deliveryNoteYear: providedDeliveryNoteYear,
  activityProfile
...
<div class="error-container">
  ...
  const tInv = deliveryNoteTranslations[selectedLanguage as keyof typeof deliveryNoteTranslations];
  ...
  const titleFontFamily = fontFamilyMap[config.deliveryNoteTitleFontFamily || "HELVETICA"];
  const clientLabelFontFamily = fontFamilyMap[config.deliveryNoteClientLabelFontFamily || "HELVETICA"];
  const clientValueFontFamily = fontFamilyMap[config.deliveryNoteClientValueFontFamily || "HELVETICA"];
  const dateFontFamily = fontFamilyMap[config.deliveryNoteDateFontFamily || "HELVETICA"];
  const tableLabelFontFamily = fontFamilyMap[config.deliveryNoteTableLabelFontFamily || "HELVETICA"];
  const tableValueFontFamily = fontFamilyMap[config.deliveryNoteTableValueFontFamily || "HELVETICA"];
  const termsFontFamily = fontFamilyMap[config.deliveryNoteTermsFontFamily || "HELVETICA"];
  const pageNumberFontFamily = fontFamilyMap[config.deliveryNotePageNumberFontFamily || "HELVETICA"];

  const lastCustomer = customers[0];
  const todayDate = getTodayDate();
  const currentYear = providedDeliveryNoteYear || getCurrentYear();
  const deliveryNoteNumber = providedDeliveryNoteNumber || getDeliveryNoteNumber();
  ...
  <div class="delivery-note-top-body">
    <div class="delivery-note-title" dir="${selectedLanguage === 'AR' ? 'rtl' : 'ltr'}">${tInv.deliveryNoteNumber} : ${deliveryNoteNumber} / ${currentYear}</div>
    ...
    <div class="date-section">
      ${config.deliveryNoteLocationDateLabelText || tInv.datePrefix} ${todayDate}
    </div>
  </div>
  `;
  ...
  <div class="items-table-wrapper" style="${maxHeightStyle}">
    ...
    <thead>
      <tr>
        <th style="width: ${config.deliveryNoteColNumberWidth ?? 13.23}mm; padding: ${(config.deliveryNoteColNumberPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.number}</th>
        <th style="width: ${config.deliveryNoteColFamilyWidth ?? 39.69}mm; padding: ${(config.deliveryNoteColFamilyPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.family}</th>
        <th style="width: ${config.deliveryNoteColModelWidth ?? 39.69}mm; padding: ${(config.deliveryNoteColModelPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.model}</th>
        <th style="width: ${config.deliveryNoteColBrandWidth ?? 31.75}mm; padding: ${(config.deliveryNoteColBrandPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.brand}</th>
        <th style="width: ${config.deliveryNoteColQuantityWidth ?? 21.17}mm; padding: ${(config.deliveryNoteColQuantityPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.quantity}</th>
        <th style="width: ${config.deliveryNoteColPriceUnitWidth ?? 31.75}mm; padding: ${(config.deliveryNoteColPriceUnitPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.priceUnit}</th>
        <th style="width: ${config.deliveryNoteColPriceTotalWidth ?? 31.75}mm; padding: ${(config.deliveryNoteColPriceTotalPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${tInv.tableHeaders.priceTotal}</th>
      </tr>
    </thead>
    <tbody>
      ${pageItems.map((item: any) => {
        if (item.type === 'model') {
          ...
          return `
      <tr>
        <td style="padding: ${(config.deliveryNoteColNumberPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${index + 1}</td>
        <td style="padding: ${(config.deliveryNoteColFamilyPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${model.family?.name || ""}</td>
        <td style="padding: ${(config.deliveryNoteColModelPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${model.name}</td>
        <td style="padding: ${(config.deliveryNoteColBrandPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${brand}</td>
        <td style="padding: ${(config.deliveryNoteColQuantityPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${quantity}</td>
        <td style="padding: ${(config.deliveryNoteColPriceUnitPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${formatPrice(pricePerUnit, selectedCurrency)} ${selectedCurrency}</td>
        <td style="padding: ${(config.deliveryNoteColPriceTotalPadding ?? 5.61) / 2}mm; text-align: center; vertical-align: middle;">${formatPrice(priceExclTax, selectedCurrency)} ${selectedCurrency}</td>
      </tr>`;
        } else {
          return `
      <tr>
        <td style="border: 1px solid #000; padding: ${(config.deliveryNoteColNumberPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.deliveryNoteColFamilyPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.deliveryNoteColModelPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.deliveryNoteColBrandPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.deliveryNoteColQuantityPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.deliveryNoteColPriceUnitPadding ?? 5.61) / 2}mm;">&nbsp;</td>
        <td style="padding: ${(config.deliveryNoteColPriceTotalPadding ?? 5.61) / 2}mm;">&nbsp;</td>
      </tr>`;
        }
      }).join('')}
    </tbody>
  ...
  const generateFooterHTML = () => {
    const signaturePosition = config.deliveryNoteSignaturePosition || "right";
    const signatureCount = config.deliveryNoteSignatureCount || 2;
    const signatureLineWidth = config.deliveryNoteSignatureLineWidth || 150;
    let signatureHTML = '';

    if (signatureCount === 1) {
      if (config.deliveryNoteIncludeLeftSignature !== false) {
      ...
      <div class="signature-line" style="${justifyContentStyle}" dir="ltr">
        <div class="signature-box" style="width: auto; text-align: center;">
          ...
          <div class="signature-label" style="text-align: center;">${config.deliveryNoteSignatureLeftLabelText || tInv.signature.manager}</div>
        </div>
      </div>`;
      }
    } else {
      if (config.deliveryNoteIncludeLeftSignature !== false || config.deliveryNoteIncludeRightSignature !== false) {
        signatureHTML = '<div class="signature-line" dir="ltr">';
        if (config.deliveryNoteIncludeLeftSignature !== false) {
          signatureHTML += `
        <div class="signature-box" style="position: relative; left: ${config.deliveryNoteLeftSignatureOffsetX || 0}px; top: ${config.deliveryNoteLeftSignatureOffsetY || 0}px;">
          <div class="signature-line-visual" style="width: ${signatureLineWidth}px;"></div>
          <div class="signature-label" style="text-align: center;">${config.deliveryNoteSignatureLeftLabelText || tInv.signature.manager}</div>
        </div>`;
        }
        if (config.deliveryNoteIncludeRightSignature !== false) {
          signatureHTML += `
        <div class="signature-box" style="position: relative; left: ${config.deliveryNoteRightSignatureOffsetX || 0}px; top: ${config.deliveryNoteRightSignatureOffsetY || 0}px;">
          <div class="signature-line-visual" style="width: ${signatureLineWidth}px;"></div>
          <div class="signature-label" style="text-align: center;">${config.deliveryNoteSignatureRightLabelText || tInv.signature.client}</div>
        </div>`;
        }
        signatureHTML += '</div>';
      }
    ...
    if (config.deliveryNoteTermsIncludeDelivery !== false) {
      const timeText = formatTimePeriod(config.deliveryNoteTermsDeliveryTimeFrom || 3, config.deliveryNoteTermsDeliveryTimeTo || null, config.deliveryNoteTermsDeliveryTimeUnit || 'months', langCode);
      termsList.push(`${tInv.terms.deliveryAfterOrder}: ${timeText} ${tInv.terms.afterOrderConfirmation.toLowerCase()}`);
    }
    if (config.deliveryNoteTermsIncludeValidity !== false) {
      const timeText = formatTimePeriod(config.deliveryNoteTermsValidityTime || 3, config.deliveryNoteTermsValidityTimeTo || null, config.deliveryNoteTermsValidityTimeUnit || 'months', langCode);
      termsList.push(`${tInv.terms.validityTitle}: ${timeText}`);
    }
    if (config.deliveryNoteTermsIncludeWarranty !== false) {
      const timeText = formatTimePeriod(config.deliveryNoteTermsWarrantyTime || 12, config.deliveryNoteTermsWarrantyTimeTo || null, config.deliveryNoteTermsWarrantyTimeUnit || 'months', langCode);
      termsList.push(`${tInv.terms.warrantyTitle}: ${timeText}`);
    }
    if (config.deliveryNoteTermsIncludeConfirmation !== false) {
      termsList.push(config.deliveryNoteTermsDeliveryCondition === 'after_payment' ? tInv.terms.afterPayment : tInv.terms.afterOrderConfirmation);
    }
    if (config.deliveryNoteTermsIncludePayment !== false) {
      const paymentMethod = config.deliveryNoteTermsPaymentMethod || 'split_payment';
      ...
      if (paymentMethod === 'split_payment') {
        const percent = config.deliveryNoteTermsSplitPaymentPercent || 50;
    ...
    if (config.deliveryNoteTermsIncludePriceInWords !== false) {
  ...
  <div class="footer-section" dir="${selectedLanguage === 'AR' ? 'rtl' : 'ltr'}">
    <div class="terms-section">
      <div class="terms-title">${config.deliveryNoteTermsTitle || tInv.terms.title}</div>
  ...
  let pagesHTML = '';
  console.log('[generatePreviewHTML - DeliveryNote] Total pages to generate:', pages.length);
  for (let i = 0; i < pages.length; i++) {
    const page = pages[i];
    const isLastPage = page.pageType === 'multi-last' || page.pageType === 'single';
    console.log(`[generatePreviewHTML - DeliveryNote] Page ${page.pageNum}:`, {
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .delivery-note-top-body { margin-bottom: ${config.factureNumberMargin ? config.factureNumberMargin / 3.78 : 0}mm; direction: ${config.deliveryNoteTopBodyDirection === 'RTL' ? 'rtl' : 'ltr'}; }
    .delivery-note-title { font-family: ${titleFontFamily}; font-size: ${config.deliveryNoteTitleFontSize || 16}px; color: ${config.deliveryNoteTitleTextColor || "#000000"}; font-weight: ${config.deliveryNoteTitleBold ? 'bold' : 'normal'}; text-decoration: ${config.deliveryNoteTitleUnderline ? 'underline' : 'none'}; margin-bottom: ${10 * (config.deliveryNoteTitleLineSpacing || 0.5) / 3.78}mm; line-height: ${config.deliveryNoteTitleLineSpacing || 0.5}; text-align: ${config.deliveryNoteTitlePosition || 'center'}; letter-spacing: ${config.deliveryNoteTitleLetterSpacing || 0}px; }
    .customer-section { display: flex; gap: 8mm; padding: 3mm; border-bottom: 1px solid #e0e0e0; text-align: ${config.deliveryNoteClientInfoPosition || 'left'}; background: #f9f9f9; }
    ...
    .customer-label { font-family: ${clientLabelFontFamily}; font-size: ${config.deliveryNoteClientLabelFontSize || 12}px; color: ${config.deliveryNoteClientLabelTextColor || "#000000"}; font-weight: ${config.deliveryNoteClientLabelBold ? 'bold' : 'normal'}; text-decoration: ${config.deliveryNoteClientLabelUnderline ? 'underline' : 'none'}; line-height: ${config.deliveryNoteClientLabelLineSpacing || 1}; letter-spacing: ${config.deliveryNoteClientLabelLetterSpacing || 0}px; }
    .customer-value { font-family: ${clientValueFontFamily}; font-size: ${config.deliveryNoteClientValueFontSize || 12}px; color: ${config.deliveryNoteClientValueTextColor || "#000000"}; font-weight: ${config.deliveryNoteClientValueBold ? 'bold' : 'normal'}; text-decoration: ${config.deliveryNoteClientValueUnderline ? 'underline' : 'none'}; line-height: ${config.deliveryNoteClientValueLineSpacing || 1}; letter-spacing: ${config.deliveryNoteClientValueLetterSpacing || 0}px; }
    .customer-field { display: flex; gap: 4mm; }
    .date-section { margin-bottom: ${config.dateMargin ? config.dateMargin / 3.78 : 1.32}mm; font-family: ${dateFontFamily}; font-size: ${config.deliveryNoteDateFontSize || 12}px; color: ${config.deliveryNoteDateTextColor || "#000000"}; font-weight: ${config.deliveryNoteDateBold ? 'bold' : 'normal'}; text-decoration: ${config.deliveryNoteDateUnderline ? 'underline' : 'none'}; line-height: ${config.deliveryNoteDateLineSpacing || 1}; letter-spacing: ${config.deliveryNoteDateLetterSpacing || 0}px; text-align: ${config.deliveryNoteDatePosition || 'left'}; }
    ...
    .items-table th, .items-table td { border: 1px solid #000; text-align: ${config.deliveryNoteTableTextAlignment || 'left'}; direction: ${config.deliveryNoteTableDirection === 'RTL' ? 'rtl' : 'ltr'}; }
    .items-table th { font-family: ${tableLabelFontFamily}; font-size: ${config.deliveryNoteTableLabelFontSize || 11}px; color: ${config.deliveryNoteTableLabelTextColor || "#000000"}; font-weight: ${config.deliveryNoteTableLabelBold ? 'bold' : 'normal'}; text-decoration: ${config.deliveryNoteTableLabelUnderline ? 'underline' : 'none'}; line-height: ${config.deliveryNoteTableLabelLineSpacing || 1}; letter-spacing: ${config.deliveryNoteTableLabelLetterSpacing || 0}px; background: #f0f0f0; }
    .items-table td { font-family: ${tableValueFontFamily}; font-size: ${config.deliveryNoteTableValueFontSize || 11}px; color: ${config.deliveryNoteTableValueTextColor || "#000000"}; font-weight: ${config.deliveryNoteTableValueBold ? 'bold' : 'normal'}; text-decoration: ${config.deliveryNoteTableValueUnderline ? 'underline' : 'none'}; line-height: ${config.deliveryNoteTableValueLineSpacing || 1}; letter-spacing: ${config.deliveryNoteTableValueLetterSpacing || 0}px; vertical-align: top; }
    ...
    .totals-table td { padding: 2.12mm; border: 1px solid #000; font-family: ${tableValueFontFamily}; font-size: ${config.deliveryNoteTableValueFontSize || 11}px; color: ${config.deliveryNoteTableValueTextColor || "#000000"}; font-weight: ${config.deliveryNoteTableValueBold ? 'bold' : 'normal'}; text-decoration: ${config.deliveryNoteTableValueUnderline ? 'underline' : 'none'}; line-height: ${config.deliveryNoteTableValueLineSpacing || 1}; letter-spacing: ${config.deliveryNoteTableValueLetterSpacing || 0}px; }
    ...
    .terms-section { margin-bottom: ${config.termsSectionMargin ? config.termsSectionMargin / 3.78 : 1.32}mm; font-family: ${termsFontFamily}; font-size: ${config.deliveryNoteTermsFontSize || 11}px; color: ${config.deliveryNoteTermsTextColor || "#000000"}; font-weight: ${config.deliveryNoteTermsBold ? 'bold' : 'normal'}; text-decoration: ${config.deliveryNoteTermsUnderline ? 'underline' : 'none'}; line-height: ${config.deliveryNoteTermsLineSpacing || 1}; letter-spacing: ${config.deliveryNoteTermsLetterSpacing || 0}px; text-align: ${config.deliveryNoteTermsPosition || 'left'}; }
    ...
    .page-number { position: absolute; bottom: ${5 + (config.deliveryNotePageNumberOffsetY || 0)}mm; ${config.deliveryNotePageNumberPosition === 'left' ? 'left: 10mm;' : ''} ${config.deliveryNotePageNumberPosition === 'right' ? 'right: 10mm;' : ''} ${config.deliveryNotePageNumberPosition === 'center' || !config.deliveryNotePageNumberPosition ? 'left: 50%; transform: translateX(-50%);' : ''} ${config.deliveryNotePageNumberPosition !== 'center' && config.deliveryNotePageNumberPosition ? `transform: translateX(${config.deliveryNotePageNumberOffsetX || 0}px);` : ''} font-family: ${pageNumberFontFamily}; font-size: ${config.deliveryNotePageNumberFontSize || 10}px; color: ${config.deliveryNotePageNumberTextColor || "#666"}; text-align: center; }
...
export const generatePDFFromHTML = async (
  previewHTML: string,
  fileName: string = `DeliveryNote_${getTodayDate().replace(/\//g, '-')}.pdf`
): Promise<void> => {
