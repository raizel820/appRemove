/**
 * POST /api/proformas/numbers/custom
 * Reserve a specific custom proforma number for a given year
 * Handles both:
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error reserving custom proforma number:', error);
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to reserve custom proforma number',
      },
      { status: 500 }
    );
  }
