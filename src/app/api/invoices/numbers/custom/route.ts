/**
 * POST /api/invoices/numbers/custom
 * Reserve a specific custom invoice number for a given year
 * Handles both:
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error reserving custom invoice number:', error);
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to reserve custom invoice number',
      },
      { status: 500 }
    );
  }
